...
public class JobStatsService {
    ...
    private Map<String, Object> processJobSparesServiceInfo(List<JobSpares> records) {
        ...
        // Total amount
        BigDecimal estimateTotal = estimateJobSpares.stream()
                .map(exp -> Optional.ofNullable(exp.getGrandTotal()).orElse(BigDecimal.ZERO))
                .reduce(BigDecimal.ZERO, BigDecimal::add);
        BigDecimal invoiceTotal = invoiceJobSpares.stream()
                .map(exp -> Optional.ofNullable(exp.getGrandTotalWithGST()).orElse(BigDecimal.ZERO))
                .reduce(BigDecimal.ZERO, BigDecimal::add);

        result.put("total", estimateTotal.add(invoiceTotal));
